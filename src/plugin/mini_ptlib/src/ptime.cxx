...
static time_t p_mktime(struct tm * t, int zone)
{
  ...
  // and then subtract
  t->tm_isdst = 0;//PTime::IsDaylightSavings() ? 1 : 0;
  time_t theTime = mktime(t);
  if (theTime == (time_t)-1)
    theTime = 0;
  else if (zone != PTime::Local) {
    theTime += 0;//PTime::GetTimeZone()*60;  // convert to local time
    if (theTime > zone*60)
...
PString PTime::AsString(TimeFormat format, int zone) const
{
  ...
  BOOL is12hour = FALSE;//GetTimeAMPM();
...
PString PTime::AsString(const char * format, int zone) const
{
  ...
  if (zone == Local)
    zone = 0;//GetTimeZone();  // includes daylight savings time
  time_t realTime = theTime + zone*60;     // to correct timezone
  //struct tm ts;
  //struct tm * t = gmtime(&realTime, &ts );
  struct tm * t = gmtime(&realTime); //ANSI
  ...
  while (*format != '\0') {
    ...
    switch (*format) {
      ...
      case 'w' :
        while (*++format == 'w')
          repeatCount++;
        //if (repeatCount != 3 || *format != 'e') //ANSI
      ...
      case 'M' :
        ...
        //else if (repeatCount > 3 || *format != 'E') //ANSI
...
{
  return 1;//PTime::GetDateOrder(); //ANSI
}


int STDAPICALLTYPE PTimeIsMonthName(const char * str, int month, int abbrev)
{
  return 0;//PTime::GetMonthName((PTime::Months)month, //ANSI
...
{
  return 0;//PTime::GetDayName((PTime::Weekdays)day, //ANSI
  //                           abbrev ? PTime::Abbreviated : PTime::FullName) *= str;
}
...
{
  time_t now;
  //struct tm timeBuf; //ANSI
  time(&now);
  microseconds = 0;
  strm >> ws;
  //theTime = PTimeParse(&strm, os_localtime(&now, &timeBuf), GetTimeZone());
  theTime = PTimeParse(&strm, localtime(&now), 0); //ANSI
}
